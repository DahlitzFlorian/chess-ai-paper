...
Im Gegenteil zu Tic-Tac-Toe, bei dem der Spielbaum maximal die Tiefe neun erreicht, kann beim Schach theoretisch eine Tiefe von 5899 erreicht werden (Quelle Wikipedia 50-Züge-Regel).
Da es im Verlauf eines Schachspiels durchschnittlich $x$ mögliche Züge für den Spieler am Zug gibt, gibt es ab einer bestimmten Tiefe $t$, $x^t$ zu analysierenden Positionen (Quelle Wikipedia Shannon number https://en.wikipedia.org/wiki/Shannon_number abgerufen 22.01.2020).
Weil sogar schnelle Computer bei diesem exponentiell wachsenden Baum irgendwann zu hohe Rechenzeiten benötigen, muss man sich auf eine maximale Tiefe festlegen bis zu der man den Spielbaum analysiert.
...
\end{itemize}
Beim Umsetzen der Kernaufgabe des Minimax-Algorithmus wird der Ansatz von (Quelle: Knuth & Moore 1975) übernommen.
Außerdem wird die Standard-Implementierung verwendet, bei denen der gesamte Algorithmus in zwei Prozeduren, Minimize und  Maximize, aufgeteilt wird.
Diese führen die dabei die Tätigkeit aus, welche die Entscheidungen des minimierenden bzw. maximierenden Spielers repräsentieren.
Bei der alternativen Negamax-Implementierung gibt es nur eine Minimax-Prozedur welche die Funktionen beider Spieler übernimmt (Quelle Wikipedia Alpha-Beta-Suche https://de.wikipedia.org/wiki/Alpha-Beta-Suche abgerufen 16.01.2020).
Um die Standard-Implementierung umzusetzten werden Funktionen $F$ und $G$ für für beide Spieler (Weiß und Schwarz)  definiert, welche das Verhalten beim Treffen auf eine Schachposition $p$ in der Tiefe $t$ definiert.
\begin{equation*}
    F(p,t) =
    \begin{cases}
        e(p) \quad \text{if }t=0,\\
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
wobei $p1$, ... , $pl$ die möglichen Positionen sind, welche nach einem Zug aus der Position p resultieren können und $e(p)$ die Eval-Function ist.
Es kann, anders als bei der Negamax-Implementierung, dieselbe Eval-Function für beide Funktionen benutzt werden, weil diese weiß-favorisierende Positionen mit positiven Werten- und schwarz-favorisierende Positionen mit negativen Werten bewertet.
